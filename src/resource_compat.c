...
size_t pak_fread(void *ptr, size_t size, size_t nmemb, PAK_FILE *stream) {
    ...
    stream->position += (uint32_t)bytes_to_read;

    return (size > 0) ? (bytes_to_read / size) : 0;
}
...
int pak_fseek(PAK_FILE *stream, long offset, int whence) {
    ...
    switch (whence) {
        case SEEK_SET:
            ...
            break;
        case SEEK_CUR:
            new_position = (long)stream->position + offset;
            break;
        case SEEK_END:
            new_position = (long)stream->size + offset;
            break;
        default:
            return -1;
    }

    if (new_position < 0 || (uint32_t)new_position > stream->size) {
        return -1;
    }

    stream->position = (uint32_t)new_position;
    return 0;
}
...
int pak_fclose(PAK_FILE *stream) {
    ...
    // Only free memory if not PAK (PAK data is owned by the global PAK system)
    if (!stream->is_pak) {
